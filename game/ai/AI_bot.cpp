...
void idAI_Bot::Event_CanFireToEnemyNoSelfDamage( int requireVisible ) {
	...
	if ( tr.c.material->GetSurfaceType() == SURFTYPE_GLASS ) { //TODO: fix this in another way... he could see me through a glass but hit another material type.
		...
		lastHitNSDCheckResult = false;
	}else if ( tr.fraction >= 1.0f || ( gameLocal.GetTraceEntity( tr ) == enemyEnt ) ) {
...
void idAI_Bot::Event_FindClosestBotNode( idEntity *ignoreNode ) {
	...
	for ( ent = gameLocal.activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next() ) {
		...
		/* 
		//make sure every distance condition is satisfied
		if ( ( botNode->DistanceContidionsTrue( this ) != DISTANCE_COND_OK ) {
			continue;
		}
		*/
		...
		if ( !gameLocal.pvs.InCurrentPVS( pvs, botNode->GetPVSAreas(), botNode->GetNumPVSAreas() ) ) {
			gameLocal.Printf("%s is not in PVS \n", botNode->GetName());
			continue;
		}
		*/
	...
	//debug only
	if(bestBotNode){
		gameLocal.Printf("%s found!\n", bestBotNode->GetName());
	}
	else{
		gameLocal.Printf("no node found!\n");
	}*/
...
void idAI_Bot::Event_SelectAnotherWeaponByNode( idEntity *node ){ 
	...
	assert( node );
	/* //don't check the type... just in case a 'lean_spot' is used...
	if( !node ){//|| !node->IsType( idBotNode::Type )
		gameLocal.Warning("SelectAnotherWeaponByNode failed: the entity is not an idBotNode!" );
		idThread::ReturnInt( false ); 
		return;
	}*/
	...
	for( wCounter = 0; wCounter < wNum; wCounter++ ) { //do exactly a complete loop (also test the current weapon so that it's ok even if we start from -1)
		...
		if ( (weapons[ testWeapon ].enabled) ){
			...
			/*
				...
				gameLocal.Printf("weapon '%s' is disabled on node '%s'!\n", testWeaponName, node->GetName() );
			}*/
		}
	}
