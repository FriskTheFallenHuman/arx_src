...
idCVar g_blobTime(					"g_blobTime",				"3",			CVAR_GAME | CVAR_FLOAT, "" );
...
idCVar r_HDR_middleGray				( "r_HDR_middleGray",			"0.16",			CVAR_GAME | CVAR_FLOAT,		" Middle gray value for overall scene . Range 0 - 1. (Works only when automatic exposure is on) ");
...
idCVar r_HDR_min_luminance			( "r_HDR_min_luminance",		"0.023",		CVAR_GAME | CVAR_FLOAT,		" Luminance is restricted to this value to control whiteness when in pitch dark areas. (Works only when automatic exposure is on)");
...
idCVar r_HDR_eyeAdjustmentDelay		( "r_HDR_eyeAdjustmentDelay",	"1.44",			CVAR_GAME | CVAR_FLOAT,		" Total time in second taken to adjust eye exposure.(Works only when automatic exposure is on)");
idCVar r_HDR_colorCurveBias			( "r_HDR_colorCurveBias",		"0.5",			CVAR_GAME | CVAR_FLOAT,		" Applies Exponential Color Curve to final pass (range 0 to 1), 1 = color curve fully applied , 0= No color curve");
idCVar r_HDR_maxColorIntensity		( "r_HDR_maxColorIntensity",	"6.5",			CVAR_GAME | CVAR_FLOAT,		" Adjusting this value will cause color burnout in a controllable fashion (range 0.0 - 256.0) \nHigher values reduces the contrast, lower values increases the contrast.");
