...
void idWeapon::GetWeaponDef( const char *objectname, int ammoinclip ) {
	...
	if ( projectileName[0] != '\0' ) {
		...
		if ( !projectileDef ) {
			gameLocal.Warning( "idWeapon::GetWeaponDef - Unknown projectile '%s' in weapon '%s'", projectileName, objectname );
		} else {
			...
			if ( !cls || !cls->IsType( idProjectile::Type ) ) {
				gameLocal.Warning( "idWeapon::GetWeaponDef - Invalid spawnclass '%s' on projectile '%s' (used by weapon '%s')", spawnclass, projectileName, objectname );
			} else {
				projectileDict = projectileDef->dict;
			}
		}
...
// Solarsplace - 23rd May 2010

/*
===============
idWeapon::magicChangeProjectileDef
===============
*/
void idWeapon::magicChangeProjectileDef( const char *projectileName ) {

	gameLocal.Printf( "idWeapon::magicChangeProjectileDef( %s )\n", projectileName );
	...
	if ( projectileName[0] != '\0' )
	{
		...
		if ( !projectileDef )
		{
			gameLocal.Warning( "idWeapon::magicChangeProjectileDef - Unknown projectile '%s' in weapon %s", projectileName, weaponDef->GetName() );
		}
		else
		{
			const char *spawnclass = projectileDef->dict.GetString( "spawnclass" );
			...
			if ( !cls || !cls->IsType( idProjectile::Type ) )
			{
				gameLocal.Warning( "idWeapon::magicChangeProjectileDef - Invalid spawnclass '%s' on projectile '%s' (used by weapon 'weapon_empty')", spawnclass, projectileName );
			}
			else
			{
				projectileDict.Clear();
	...
	{
		gameLocal.Printf( "idWeapon::magicChangeProjectileDef - projectileName[0] missing.\n", projectileName[0] );
	}
